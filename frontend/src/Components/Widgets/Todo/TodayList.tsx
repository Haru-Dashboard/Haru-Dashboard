  ...
  */
  ...
  const filterTodayList = () => {
    if (clickedCategory === '전체') {
      ...
      const filtered = localTodayList.filter(today => today.category === clickedCategory)
  ...
  return (
    <div style={{height: '50%', fontSize: '0.8rem' }}>
      ...
      <div className='sub-board mx-3' style={{height: '80%', overflowY: 'auto', overflowX: 'hidden'}}>
        {/* 작성한 todo가 보이는 곳 */}
        {!isEmpty && (
        ...
        {isEmpty && (
          <p className='text-center'>{clickedCategory}에서 할 일을 추가해주세요!</p>
        )}
      </div>     
    </div>
  );
